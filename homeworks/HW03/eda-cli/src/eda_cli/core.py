...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    - HW03: константные колонки;
    - HW03: подозрительно много нулей (эвристика mean/max).
    """
    flags: Dict[str, Any] = {}
    ...
    # HW03: эвристика 1 — константные колонки
    ...
    # HW03: эвристика 2 — подозрительно много нулей (через mean/max)
    suspicious_zero_columns: List[str] = []
    for c in summary.columns:
        if not c.is_numeric:
            continue
        if c.max is None or c.mean is None:
            continue
        if c.max != 0 and (c.mean / c.max) < 0.1:
            suspicious_zero_columns.append(c.name)

    flags["has_many_zero_values"] = len(suspicious_zero_columns) > 0
    flags["many_zero_value_columns"] = suspicious_zero_columns

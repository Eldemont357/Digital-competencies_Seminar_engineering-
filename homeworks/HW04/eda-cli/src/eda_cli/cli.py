...
def report(
    ...
    """
    Сгенерировать полный EDA-отчёт с параметрами HW03.
    """
    ...
    # HW03: проблемные колонки по пропускам
    problem_cols = []
    if not missing_df.empty:
        problem_cols = missing_df[
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"- Подозрительно много нулей: **{quality_flags['has_many_zero_values']}**\n")
        f.write(f"  - {quality_flags['many_zero_value_columns']}\n\n")
        ...
        if missing_df.empty:
            ...
            if problem_cols:
                ...
                f.write("Колонок с пропусками выше порога не найдено.\n\n")
